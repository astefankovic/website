...
class db
{
          ...
          $where_pom = "";
	 			 	...
	 			 	if ($insert != "") $insert .= ", ";
	 			 		...
	 			 		if (key_exists("value", $row))
	 			 				...
	 			 				$insert .= "`$column` = $value_pom";
	 			 	}
	
      		if ($where_array != null)
    			foreach ($where_array as $index_where => $item_where)
    			{
    				// pridat AND
    				if ($where_pom != "") $where_pom .= "AND ";
    	
    				$column_where = $item_where["column"];
    				$symbol_where = $item_where["symbol"];
    	
    				if (key_exists("value", $item_where))
    					$value_pom_where = "?"; 						// budu to navazovat
    				else if (key_exists("value_mysql", $item_where))
    					$value_pom_where = $item_where["value_mysql"]; 		// je to systemove, vlozit rovnou - POZOR na SQL injection, tady to muze projit
    	
    	
    				$where_pom .= "`$column_where` $symbol_where $value_pom_where";
    			}
    			...
    			if (trim($where_pom) != "") $where_pom = "where $where_pom";
	 			 	...
	 			 	// 1) pripravit dotaz s dotaznikama
	 			 				$query = "update `$table_name` set $insert $where_pom;";
	 			 				//echo $query;
  ...
  public function DBDelete($table_name, $item, $where_array)
	{
	 			 // MySql
	       // vznik chyby v PDO
		      $mysql_pdo_error = false;
